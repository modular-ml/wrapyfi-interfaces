...
ORIENTATION_FILES = ["results/benchmarking_orientation_interfaces__zeromq,zeromq_pitch.csv",
                     "results/benchmarking_orientation_interfaces__zeromq,zeromq_yaw.csv",
                     "results/benchmarking_orientation_interfaces__zeromq,zeromq_roll.csv",
                     "results/benchmarking_orientation_interfaces__yarp,yarp_pitch.csv",
                     "results/benchmarking_orientation_interfaces__yarp,yarp_yaw.csv",
                     "results/benchmarking_orientation_interfaces__yarp,yarp_roll.csv",
                     "results/benchmarking_orientation_interfaces__ros,ros_pitch.csv",
                     "results/benchmarking_orientation_interfaces__ros,ros_yaw.csv",
                     "results/benchmarking_orientation_interfaces__ros,ros_roll.csv",
                     "results/benchmarking_orientation_interfaces__ros2,ros2_pitch.csv",
                     "results/benchmarking_orientation_interfaces__ros2,ros2_yaw.csv",
                     "results/benchmarking_orientation_interfaces__ros2,ros2_roll.csv",
                     ]
...
for ORIENTATION_FILE in ORIENTATION_FILES:
    ...
    pitch_1 = df_1["pitch"].values.copy()
    pitch_1.resize(300, refcheck=False)
    yaw_1 = df_1["yaw"].values.copy()
    yaw_1.resize(300, refcheck=False)
    roll_1 = df_1["roll"].values.copy()
    roll_1.resize(300, refcheck=False)

    df_2 = df[df["topic"] == "orientation_sixdrepnet"]
    pitch_2 = df_2["pitch"].values.copy()
    pitch_2.resize(300, refcheck=False)
    yaw_2 = df_2["yaw"].values.copy()
    yaw_2.resize(300, refcheck=False)
    roll_2 = df_2["roll"].values.copy()
    roll_2.resize(300, refcheck=False)

    x = np.arange(0, 300)
    if "yaw" in ORIENTATION_FILE:
        s1 = yaw_2
        s2 = yaw_1
    elif "roll" in ORIENTATION_FILE:
        s1 = roll_2
        s2 = roll_1
    elif "pitch" in ORIENTATION_FILE:
        s1 = pitch_2
        s2 = pitch_1
    ...
    def plot_warpingpaths(s1, s2, paths, path=None, filename=None, shownumbers=False, showlegend=False,
        ...
        if p is not None:
            ...
            distance = paths[p[-1][0] + 1, p[-1][1] + 1]
        ...
        ax1.plot(range(len(s2)), s2, ".-", lw=5, color="blue")
        ...
        ax2.plot(-s1, range(max_s1_y, 0, -1), ".-", lw=5, color="green")
        ...
        if filename:
            filename = filename.replace("{distance}", str(np.round(distance, 2)))
    ...
    plot_warpingpaths(s1, s2, paths, best_path, filename=ORIENTATION_FILE.replace(".csv", "") + "__W_distance_{distance}.png")

    path = dtw.warping_path(s1, s2)
    dtwvis.plot_warping(s1, s2, path, filename=ORIENTATION_FILE.replace(".csv", "") + "__distance_" + str(np.round(distance, 2)) + ".png")
